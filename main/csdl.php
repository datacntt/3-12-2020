...
function menudanhmuc()
{
	...
	if (mysqli_num_rows($kq) > 0) {
		...
		while ($row = mysqli_fetch_array($kq)) {
			$chuoi .= "<a class='dropdown-item' href='danhmuc.php?iddm=$row[0]'>$row[1]</a>";
...
{
	$conn = ketnoi();
	// $sql = "select idtl,tentl,danhmuc.path from tailieu inner join danhmuc on tailieu.iddm=danhmuc.iddm order by idtl desc limit 5";
	$sql = "select idtl,tentl,danhmuc.path from tailieu inner join danhmuc on tailieu.iddm=danhmuc.iddm order by idtl desc limit 5";
	$kq = mysqli_query($conn, $sql);
	if (mysqli_num_rows($kq) > 0) {
		$tlm = "<div class='tieude' style='margin-top:36px;'>
		<div class='icontieude'><img src='images/icons8-new-100.png'></div>
...
function tailieunoibat()
{
	...
	if (mysqli_num_rows($kq) > 0) {
		...
		<div class='icontieude'><img src='images/icons8-document-128.png'></div>
